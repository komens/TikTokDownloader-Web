...
class Parameter:
    ...
    def __init__(
        ...
        # 以下参数为兼容旧 settings.json 与 Parameter 内部 TikTok/终端模式逻辑保留
        # 精简版 settings.json 不会传递这些参数，使用默认值即可
        ...
        tiktok_platform: bool = False,
        browser_info_tiktok: dict | None = None,
        **kwargs,
    ):
        ...
        self.accounts_urls_tiktok: list[SimpleNamespace] = self.check_urls_params(
            accounts_urls_tiktok or []
        )
        self.mix_urls: list[SimpleNamespace] = self.check_urls_params(mix_urls or [])
        self.mix_urls_tiktok: list[SimpleNamespace] = self.check_urls_params(
            mix_urls_tiktok or []
        )
        self.owner_url: SimpleNamespace = self.check_url_params(owner_url or {})
        ...
        self.queue_retention_days = max(0, int(queue_retention_days))
    ...
    def get_settings_data(self) -> dict:
        return {
            ...
            "queue_retention_days": self.queue_retention_days,
